...
import { updateOneItem } from '../../actions/itemsActions'
...
const EditItemForm = ({ item, confirm, handleError, cancel }: ItemFormProps) => {
  ...
  const submitHandler = (values: Item) => {
    dispatch(updateOneItem({ ...values, id: item.id }))
      .then((resp: any) => {
        if (resp.error) {
